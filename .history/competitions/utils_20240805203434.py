...
from .models import League, Championship, ChampionshipParticipation, Season, Country
...
def create_or_get_current_season():
    ...
    if not current_season:
        current_season = Season.create_next_season()
...
def add_club_to_championship(club):
    ...
    if not lowest_league:
        ...
        lowest_league = League.objects.create(name=f"{country.name} League 1", country=country, level=1)
    ...
    if participations.count() < 18:
        ChampionshipParticipation.objects.create(championship=championship, club=club)
    else:
        # Создаем новую лигу уровнем ниже
        new_league = League.objects.create(
            name=f"{lowest_league.country.name} League {lowest_league.level + 1}",
            country=lowest_league.country,
